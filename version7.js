...
function setup() {
  ...
  simulationSpeedSlider = createSlider(1, 10, 5);
  simulationSpeedSlider.position(10, 90);
  simulationSpeedSlider.style('width', '80px');
}
...
function draw() {
  ...
  let speed = simulationSpeedSlider.value();
  ...
  for (let creature of creatures) {
    ...
    // Avoid predators
    for (let predator of predators) {
      let d = dist(creature.position.x, creature.position.y, predator.position.x, predator.position.y);
      if (d < 50) {
        let steer = p5.Vector.sub(creature.position, predator.position);
        steer.setMag(0.1);
        creature.velocity.add(steer);
      }
    }
  }
...
class Creature {
  constructor(x, y) {
    ...
    this.age = 0;
    this.maxAge = int(random(100, 500));
    this.color = color(random(255), random(255), random(255));
    this.randomMoveInterval = int(random(60, 180));
    this.lastRandomMoveTime = millis();
  }
  ...
  update() {
    ...
    // Transfer energy to nearby creatures
    for (let other of creatures) {
      let d = dist(this.position.x, this.position.y, other.position.x, other.position.y);
      if (d < 20 && this.energy > other.energy) {
        let transferAmount = min(this.energy - 1, 1);
    ...
    // Random movement
    if (millis() - this.lastRandomMoveTime > this.randomMoveInterval) {
      this.velocity = p5.Vector.random2D();
      this.velocity.setMag(random(1, 3));
      this.lastRandomMoveTime = millis();
    }
...
class Predator {
  constructor(x, y) {
    ...
    this.energy = 100;
    this.satiety = 0;
    this.color = color(random(100, 200), random(100, 200), random(200, 255));
    this.randomMoveInterval = int(random(60, 180));
    this.lastRandomMoveTime = millis();
  }
  ...
  reproduce() {
    let newPredator = new Predator(this.position.x, this.position.y);
    newPredator.energy /= 2;
