...
function setup() {
  ...
  // Create checkbox for predator boundary wrapping
  predatorBoundaryCheckbox = createCheckbox('Predator Boundary Wrap', false);
  predatorBoundaryCheckbox.position(10, 70);

  // Create checkbox for creature and predator boundary wrapping
  boundaryWrapCheckbox = createCheckbox('Boundary Wrap', false);
  boundaryWrapCheckbox.position(10, 90);
}
...
function draw() {
  ...
  for (let i = predators.length - 1; i >= 0; i--) {
    predators[i].update();
    predators[i].display();
    if (predators[i].isDead() || (predatorBoundaryCheckbox.checked() && boundaryWrapCheckbox.checked())) {
